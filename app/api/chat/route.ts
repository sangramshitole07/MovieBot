...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // STEP 2.5: If we have context, use similarity scores to rank it better
      if (context.length > 0) {
        try {
          console.log('Ranking context using similarity scores...');
          const similarityScores = await generateSimilarityScores(query, context);
          
          // Combine context with similarity scores and sort by relevance
          ...
          console.log('Successfully ranked context by similarity.');
        } catch (rankingError) {
          console.warn('Failed to rank context, using original order:', rankingError);
          // Continue with original context if ranking fails
        }
      }
    } catch (error) {
